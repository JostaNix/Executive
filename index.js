...
client.on("message", async msg => {
  ...
  if (msg.channel.id == auth.GuildData.GChat && !msg.author.bot && auth.GuildData.Prefixes.some(x => !msg.content.startsWith(x))) {
    ...
    if (iltifat >= 100) {
      iltifat = 0;
      let nixltifat = iltifatlar[Math.floor(Math.random() * iltifatlar.length)];
      msg.reply(nixltifat);
    }
  }
});
...
const RoleDatabase = require("./Models/Rol.js")
  client.on("guildMemberUpdate", async(oldMember, newMember) =>{
    let aldiverdi;
    if(oldMember.roles.cache.size < newMember.roles.cache.size){ aldiverdi = "" } else { aldiverdi = ""}
    ...
    let nix = await RoleDatabase.findOne({ guildID: newMember.guild.id, kullanıcıID: newMember.id }) 
    if(!nix){
      let newRoleData = new RoleDatabase({
        ...
        nix.rolveridb.push({ staffID: rolveren.executor.id, tarih: Date.now(), rolid: role.id, type: aldiverdi })
        nix.save()
      }
  }
    })
